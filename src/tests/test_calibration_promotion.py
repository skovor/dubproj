...
class PromotionTests(unittest.TestCase):
    ...
    def test_dataset_hashes_are_recomputed(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            lid_file = root / "lid.json"; lid_payload = {**artifact.to_dict(), "feature_schema_version": "lid-fusion-v3", "features": list(LID_FEATURES), "coefficients": [0.0] * len(LID_FEATURES), "normalization": [{"mean": 0.0, "scale": 1.0} for _ in LID_FEATURES]}; lid_file.write_text(json.dumps(lid_payload), encoding="utf-8"); lid_sha = hashlib.sha256(lid_file.read_bytes()).hexdigest()
            ...
            profile = promote_profile(profile_id="x", target_artifact={**artifact.to_dict(), "artifact_path": str(artifact_file), "artifact_sha256": sha}, final_anchor_artifact={**final_payload, "artifact_path": str(final_file), "artifact_sha256": final_sha}, lid_artifact={**lid_payload, "artifact_path": str(lid_file), "artifact_sha256": lid_sha}, validation=valid, hidden=hidden, validation_rows=validation_rows, hidden_rows=hidden_rows, role_validation_rows={"target": validation_rows, "final_anchor": anchor_validation, "lid": lid_validation}, role_hidden_rows={"target": hidden_rows, "final_anchor": anchor_hidden, "lid": lid_hidden}, role_validation_reports={"target": valid, "final_anchor": anchor_valid, "lid": lid_valid}, role_hidden_reports={"target": hidden, "final_anchor": anchor_hidden_report, "lid": lid_hidden_report}, dataset_files=files, identity={"backend_id":"b","model_id":"m","model_revision":"r","feature_schema_version":"char-alignment-v3","target_language":"de","source_language":"en","performance_modes":["NEUTRAL"]}, thresholds={"target_pass_probability":.8,"target_failure_probability":.2,"final_anchor_pass_probability":.8,"source_lid_probability":.8}, provenance={"code_commit":"a"*40,"runtime_lock_sha256":"1"*64,"models_lock_sha256":"2"*64}, output=root/"profile.json")
