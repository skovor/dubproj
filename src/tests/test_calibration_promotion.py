...
class PromotionTests(unittest.TestCase):
    ...
    def test_dataset_hashes_are_recomputed(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            final_file = root / "final-anchor.json"; final_file.write_text(json.dumps({**artifact.to_dict(), "feature_schema_version": "final-anchor-v1", "features": ["final_coverage"], "coefficients": [1.0]}), encoding="utf-8"); final_sha = hashlib.sha256(final_file.read_bytes()).hexdigest(); files = {}
            ...
            profile = promote_profile(profile_id="x", target_artifact={**artifact.to_dict(), "artifact_path": str(artifact_file), "artifact_sha256": sha}, final_anchor_artifact={**artifact.to_dict(), "feature_schema_version": "final-anchor-v1", "features": ["final_coverage"], "coefficients": [1.0], "artifact_path": str(final_file), "artifact_sha256": final_sha}, validation=valid, hidden=hidden, dataset_files=files, identity={"backend_id":"b","model_id":"m","model_revision":"r","feature_schema_version":"char-alignment-v2","target_language":"de","source_language":"en","performance_modes":["NEUTRAL"]}, thresholds={"target_pass_probability":.8,"target_failure_probability":.2,"final_anchor_pass_probability":.8,"source_lid_probability":.8}, provenance={"code_commit":"c","runtime_lock_sha256":"0"*64,"models_lock_sha256":"0"*64}, output=root/"profile.json")
