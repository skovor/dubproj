...
def execute_second_game_adapter(descriptor: dict, repository_root: Path) -> dict:
    ...
    try:
        ...
        evidence=adapter.validate(manifest)
...
def main()->int:
    ...
    if not ((benchmark.get("attestation") or {}).get("verified") is True): errors.append({"code":"BENCHMARK_SELF_HASH_NOT_ATTESTATION"})
    ...
    for line_id, row in zip(manifest.line_ids, rows):
        row_evidence=verify_benchmark_row(line_id, row)
        if not row_evidence["valid"]: errors.append({"code":"BENCHMARK_ROW_ARTIFACT_UNVERIFIED","line_id":line_id,"details":row_evidence})
    second=execute_second_game_adapter(second_descriptor, Path(__file__).resolve().parents[1])
