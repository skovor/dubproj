...
def _preflight(config: PipelineConfig, manifest: str) -> dict:
    scenes = normalize_manifest(json.loads(Path(manifest).read_text(encoding="utf-8")))
    result = {"schema": "dubbing-pipeline-preflight-v2", "lab_mode": bool(config.lab_mode), "manifest": str(Path(manifest).resolve()), "scene_count": len(scenes), "line_count": sum(len(item["lines"]) for item in scenes), "phases": list(PHASES), "roots": {"project_root": str(config.project_root), "output_root": str(config.output_root), "cache_root": str(config.cache_root)}}
    if config.lab_mode:
...
def main(argv: list[str] | None = None) -> int:
    ...
    preflight = sub.add_parser("preflight"); preflight.add_argument("--config", required=True); preflight.add_argument("--manifest", required=True)
    ...
    if args.command == "preflight":
        print(json.dumps(_preflight(config, args.manifest), ensure_ascii=False, indent=2)); return 0
