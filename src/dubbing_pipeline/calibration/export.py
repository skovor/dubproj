...
def load_draft(path: str | Path) -> dict[str, Any]:
    ...
    if value.get("feature_schema_version") != FEATURE_SCHEMA_VERSION or value.get("normalization_version") != NORMALIZATION_VERSION: raise ValueError("calibration schema mismatch")
    features = list(value.get("features") or []); coefficients = list(value.get("coefficients") or [])
    if len(features) != len(coefficients) or not all(math.isfinite(float(x)) for x in coefficients + [value.get("intercept", 0.0)]): raise ValueError("invalid coefficients")
    return value
