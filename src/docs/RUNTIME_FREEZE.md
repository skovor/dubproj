...
`backend_version` and `files`. Never use `unknown`, `main`, `latest` or a null
revision in a production lock.
...
will return a non-zero status until every required identity is pinned. Model
revision changes are part of ASR/alignment cache keys, so changing a lock
